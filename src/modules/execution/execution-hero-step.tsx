...
export function ExecutionHeroStep({
  ...
  const instruction = current.description;

  return (
    <motion.section
      className="mx-4 mt-5 rounded-2xl border border-[#1F2937] bg-gradient-to-br from-[#111827] to-[#0B0F19] p-6 shadow-lg shadow-[#3B82F6]/5"
      initial={reduced ? false : { opacity: 0, scale: 0.98 }}
      animate={{ opacity: 1, scale: 1 }}
      key={current.id}
    >
      <p className="text-[11px] font-medium uppercase tracking-widest text-[#06B6D4]">
        Etapa {completedCount + 1} de {steps.length}
      </p>
